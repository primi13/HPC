...
// compile as: gcc adaptquad_par1.c -fopenmp -lm -o adaptquad_par1
...
double quad_par(double (*f)(double), double lower, double upper, double tol) {
    ...
    if (eps > tol) {
        #pragma omp task shared(quad_lower)     // create another task for lower half of interval
        quad_lower = quad_par(f, lower, middle, tol / 2);

        #pragma omp task shared(quad_upper)     // create another task for upper half of interval
