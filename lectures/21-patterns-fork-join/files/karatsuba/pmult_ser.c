...
int karatsuba_mult(double *c, double *a, double *b, int n, int cutoff) {
    if (n <= cutoff)
        simple_mult(c, a, b, n);
    else {
        int m = n/2;
        ...
        // c[0] = tL = aL*bL
        karatsuba_mult(&c[0], &a[0], &b[0], m, cutoff);
        // c[2*m] = tH = aH*bH
        karatsuba_mult(&c[2*m], &a[m], &b[m], n-m, cutoff);
        ...
        // tLH = aLH*bLH
        karatsuba_mult(t, a_, b_, n-m, cutoff); // n-m >= m; n-m = m, or m+1
