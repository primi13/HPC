/* 
	...
	No deadlock, but not efficient solution
	
	gcc -fopenmp phil2.c -o phil2
	srun --reservation=fri --cpus-per-task=5 phil2
*/
...
void discussion(void) {
	...
	while (course < COURSES) {
		...
		omp_set_lock(&mutex_taking);
		omp_set_lock(&mutex_forks[p]);
		usleep(1000);
		omp_set_lock(&mutex_forks[(p+1)%P]);
		omp_unset_lock(&mutex_taking);
