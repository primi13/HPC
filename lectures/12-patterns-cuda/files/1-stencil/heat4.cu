...
//      d2T/dx2 + d2T/dy2 = 0
// compile and run:
//      nvcc -o heat4 heat4.cu
//      srun --reservation=fri --partition=gpu --gpus=1 ./heat4 512
...
__global__ void heatStep(float* surfaceOut, float* surfaceIn, int width, int height) {
    ...
    int gy = blockIdx.x * blockDim.x + threadIdx.x + 1;
    int gx = blockIdx.y * blockDim.y + threadIdx.y + 1;
    int idx = gy * width + gx;

    int interior = gx > 0 && gx < width - 1 && gy > 0 && gy < height - 1;

    int ty = threadIdx.x + 1;   // tile indices 0 .. BLOCK_SIZE + 1
